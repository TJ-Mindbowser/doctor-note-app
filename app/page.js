...
export default function Home() {
  ...
  const handleAddNote = (noteObj, action) => {
    ...
    userData[0].PatientNote.push(noteData);
    if (userData[0]?.recentNotes) {
      userData[0]?.recentNotes.push(noteData);
    } else {
  ...
  return (
    <main>
      {isLoading ? (
        ...
        <div className="wrapper flex h-screen items-stretch justify-between m-20 border border-gray-100 rounded-xl">
          <Patients
            data={notes}
